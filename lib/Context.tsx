...
type contextType = {
  ...
  nftUrl: string[] | null;
  setNftUrl: React.Dispatch<React.SetStateAction<string[] | null>>;
};
...
export const DataProvider = ({ children }: Props) => {
  ...
  // First render
  // Get NFT collection from server
  useEffect(() => {
    axios
  ...
  // Check is there selected NFT on Step 3
  useEffect(() => {
  ...
  const postDataStep2 = async (url: string, data: ParsedUrlQuery) => {
